        ...
        for bbox in result.boxes:
            ...
            if label.lower() in ['car', 'motorcycle'] and confidence >= 0.85:
                ...
                for box in detections:
                    ...
                    _, binary_image = cv2.threshold(contrast_adjusted2, 180, 255, cv2.THRESH_BINARY)

                    # 使用pytesseract進行OCR辨識
                    text = pytesseract.image_to_string(rotated_plate, lang='eng', config='--psm 8')
                    text_con = pytesseract.image_to_string(contrast_adjusted, lang='eng', config='psm 8')
                    text_binary = pytesseract.image_to_string(binary_image, lang='eng', config='--psm 8')
                    text_eroded = pytesseract.image_to_string(eroded, lang='eng', config='--psm 8')
                    ...
                    print(f"Detected license plate number binary: {filtered_text_eroded.strip()}")


                    # 在車牌位置上顯示車牌號碼
                    cv2.putText(frame, text.strip(), (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.putText(frame, filtered_text_con.strip(), (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.putText(frame, filtered_text_binary.strip(), (x1 - 20, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.putText(frame, filtered_text_eroded.strip(), (x1 - 60, y1 - 70), cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 255, 0), 2)
